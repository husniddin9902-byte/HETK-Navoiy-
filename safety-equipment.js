(function(){
  ...
  function geographyFromFolder(id){
    const chain=folderChain(id),names=chain.map(x=>x.name).filter(Boolean);if(!names.length)return {region:'Hudud ko‘rsatilmagan',district:'Tuman ko‘rsatilmagan'};
    const district=[...names].reverse().find(n=>/(tuman|shahar|\btet\b|elektr\s*tarmoq)/i.test(n))||names[names.length-1];
    const region=names.find(n=>/(viloyat|hududiy|\bhetk\b|\bhet\b)/i.test(n)&&n!==district)||(names.length>1?names[Math.max(0,names.length-2)]:names[0]);
  ...
  function itemGeography(item){
    ...
    const zone=zones[id]||{},root=zoneRoots(zone)[0],geo=geographyFromFolder(root);return {region:geo.region==='Hudud ko‘rsatilmagan'?(zone.region||'Hudud ko‘rsatilmagan'):geo.region,district:geo.district==='Tuman ko‘rsatilmagan'?(zone.region||'Tuman ko‘rsatilmagan'):geo.district};
  }
  ...
  function visibleFolderRows(){
    ...
    const preferred=rows.filter(x=>x.hasChildren&&/(tuman|shahar|\btet\b|elektr\s*tarmoq)/i.test(x.name));
    const chosen=preferred.length?preferred:rows.filter(x=>x.hasChildren),seen=new Set(chosen.map(x=>x.id));
    ...
    Object.keys(norms).forEach(id=>{const norm=norms[id]||{},row=norm.unitType==='dispatcher'&&rows.find(x=>x.id===norm.unitId);if(row&&!seen.has(row.id)){chosen.push(row);seen.add(row.id);}});
  ...
  function normRecordForKey(key){const found=Object.keys(norms).find(id=>norms[id]&&norms[id].unitKey===key);return found?Object.assign({id:found},norms[found]||{}):null;}
  function unitMeta(type,id,fallback){
    ...
    if(type==='dispatcher'){
      ...
      return {key:unitKey(type,id),type,id,name:/dispetcher/i.test(raw)?raw:`${raw} dispetcherligi`,region:geo.region!=='Hudud ko‘rsatilmagan'?geo.region:(base.regionName||geo.region),district:geo.district!=='Tuman ko‘rsatilmagan'?geo.district:(base.districtName||geo.district)};
    }
    const zone=zones[id]||{},geo=itemGeography({workZoneId:id});return {key:unitKey(type,id),type:'work_zone',id,name:zone.name||base.unitName||'Biriktirilmagan U/J',region:geo.region||base.regionName||'Hudud ko‘rsatilmagan',district:geo.district||base.districtName||'Tuman ko‘rsatilmagan'};
  }
  function analyticsUnitRows(){
    ...
    Object.keys(norms).forEach(id=>{const row=norms[id]||{},type=row.unitType==='dispatcher'?'dispatcher':'work_zone',unitId=row.unitId||'';if(!unitId||(isMaster()&&(type==='dispatcher'||(unitId!==me.workZoneId&&(!zones[unitId]||zones[unitId].currentMasterUid!==me.uid)))))return;const allowed=type==='dispatcher'?accountCoversFolder(me,unitId):accountCoversZone(me,unitId);if(allowed&&!map.has(row.unitKey||unitKey(type,unitId)))map.set(row.unitKey||unitKey(type,unitId),unitMeta(type,unitId,row));});
  ...
  function unitStats(meta){
    const related=coveredItems().filter(item=>itemUnitKey(item)===meta.key),active=related.filter(item=>itemState(item).key!=='archived'),archived=related.filter(item=>itemState(item).key==='archived'),norm=normRecordForKey(meta.key),quantities=norm&&norm.quantities||{},activeBy={},archiveBy={};
  ...
  function uniqueNames(rows,key){return Array.from(new Set(rows.map(row=>row[key]).filter(Boolean))).sort((a,b)=>a.localeCompare(b,'uz'));}
  ...
  function analyticsTable(rows){
    ...
    return `<div class="hetk-se-tablewrap analytics"><table class="hetk-se-table"><thead><tr><th>№</th><th>${title}</th><th>Turi</th><th>Faol</th><th>Arxiv</th><th>Vositalar tarkibi</th><th>Me’yor</th><th>Yetishmaydi</th><th>Kam vositalar</th><th></th></tr></thead><tbody>${rows.map((row,i)=>`<tr class="${row.shortage?'has-shortage':''}"><td class="num">${i+1}</td><td class="hetk-se-zone"><b>${esc(row.name)}</b><small>${esc(analyticsLevel==='region'?'Viloyat kesimi':analyticsLevel==='district'?row.region:row.district)}</small></td><td><span class="hetk-se-type-tag ${row.type}">${row.type==='mixed'?'Umumiy':esc(unitTypeLabel(row.type))}</span></td><td><b class="hetk-se-number blue">${row.active}</b></td><td><b class="hetk-se-number gray">${row.archived}</b></td><td>${equipmentDetails(row)}</td><td><b class="hetk-se-number green">${row.normTotal}</b>${!row.configured?'<small class="hetk-se-unset">kiritilmagan</small>':''}</td><td><b class="hetk-se-number ${row.shortage?'red':'green'}">${row.shortage}</b></td><td>${shortageDetails(row)}</td><td>${analyticsLevel==='unit'&&canManageCatalog()?`<button class="hetk-se-iconbtn" data-se-norm-unit="${attr(row.key)}" title="Me’yoriy sonni belgilash"><i class="fas fa-sliders"></i></button>`:''}</td></tr>`).join('')}</tbody></table></div>`;
  }
  ...
  function renderAnalytics(){
    ...
    main.innerHTML=`<section class="hetk-se-analytics-hero"><div><small>HIMOYA VOSITALARI TA’MINOTI</small><h3>Me’yor, mavjud vositalar va kamchiliklar</h3><p>U/J va dispetcherliklar alohida hisoblanadi. Natijani bo‘linma, tuman yoki viloyat kesimida ko‘ring.</p></div>${canManageCatalog()?'<button class="hetk-se-btn primary" data-se-norm-add><i class="fas fa-sliders"></i>Me’yor belgilash</button>':''}</section>
  ...
  function normQuantitiesHtml(key){
    const record=normRecordForKey(key),qty=record&&record.quantities||{},rows=Object.keys(catalog).map(id=>Object.assign({id},catalog[id]||{})).filter(x=>x.active!==false).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'uz'));
  ...
  function normForm(selectedKey){
    if(!canManageCatalog())return;const rows=Object.keys(catalog).filter(id=>catalog[id]&&catalog[id].active!==false);if(!rows.length)return toast('Avval umumiy ro‘yxatga himoya vositalarini kiriting.','error');const units=unitOptionRows();if(!units.length)return toast('U/J yoki dispetcherlik topilmadi.','error');const key=selectedKey&&units.some(x=>x.key===selectedKey)?selectedKey:units[0].key;
    const content=`<header><i class="fas fa-sliders"></i><div><h3>Me’yoriy sonni belgilash</h3><p>Tanlangan U/J yoki dispetcherlik uchun har bir vositaning zarur sonini kiriting.</p></div><button data-se-modal-close>×</button></header><div class="hetk-se-dialog-body"><div id="hetk-se-formerror" class="hetk-se-formerror"></div><div class="hetk-se-field full"><span>Bo‘linma *</span><select id="hetk-se-norm-unit">${unitOptions(key,'')}</select></div><div class="hetk-se-norm-head"><b>Umumiy ro‘yxatdan me’yor tanlash</b><span>0 — me’yor belgilanmagan</span></div><div id="hetk-se-norm-rows" class="hetk-se-norm-rows">${normQuantitiesHtml(key)}</div><div class="hetk-se-field full"><span>Maxsus tahrirlash kodi *</span><input id="hetk-se-norm-code" type="password" inputmode="numeric" autocomplete="off" placeholder="Umumiy ro‘yxat uchun o‘rnatilgan kod"><small>${settings.catalogEditCodeHash?'Amaldagi maxsus kodni kiriting.':'Birinchi saqlashda shu kod o‘rnatiladi (kamida 4 belgi).'}</small></div></div><footer><button class="cancel" data-se-modal-close>Bekor qilish</button><button id="hetk-se-norm-save" class="save"><i class="fas fa-save"></i> Me’yorni saqlash</button></footer>`;openModal(content,true);byId('hetk-se-norm-unit').addEventListener('change',e=>{const box=byId('hetk-se-norm-rows');if(box)box.innerHTML=normQuantitiesHtml(e.target.value);});byId('hetk-se-norm-save').addEventListener('click',saveNorm);
  }
  ...
  async function saveNorm(){
    if(!canManageCatalog())return;const selected=fieldValue('hetk-se-norm-unit'),unit=parseUnitSelection(selected),btn=byId('hetk-se-norm-save'),quantities={};document.querySelectorAll('[data-se-norm-catalog]').forEach(input=>{const value=Math.max(0,Math.floor(Number(input.value)||0));if(value>0)quantities[input.dataset.seNormCatalog]=value;});setBusy(btn,true);
    try{const verified=await verifyCatalogCode(fieldValue('hetk-se-norm-code'),true),meta=unitMeta(unit.type,unit.id),existing=normRecordForKey(selected),id=existing&&existing.id||db.ref('SafetyEquipmentNorms').push().key,before=existing||{},beforeQty=before.quantities||{},stamp=now(),after={id,unitKey:selected,unitType:unit.type,unitId:unit.id,unitName:meta.name,regionName:meta.region,districtName:meta.district,quantities,updatedAt:stamp,updatedBy:me.uid,updatedByName:userName(me.uid),updatedByRole:roleLabel(me),createdAt:before.createdAt||stamp,createdBy:before.createdBy||me.uid},changes=normDiff(beforeQty,quantities),updates={};if(!changes.length&&existing)throw new Error('Me’yoriy sonlarda o‘zgarish yo‘q.');updates[`SafetyEquipmentNorms/${id}`]=after;if(verified.isNew)updates['SafetyEquipmentSettings/catalogEditCodeHash']=verified.hash;appendAudit(updates,'norm',id,existing?'norm_update':'norm_create',`${meta.name} himoya vositalari me’yori ${existing?'yangilandi':'belgilandi'}`,before,after,unit.type==='work_zone'?unit.id:'',changes,{unitType:unit.type,unitName:meta.name});await db.ref().update(updates);await notifyUsers(allTbUids(),{action:existing?'safety_equipment_norm_update':'safety_equipment_norm_create',title:`${meta.name} uchun himoya vositalari me’yori ${existing?'yangilandi':'belgilandi'}`,item:{equipmentName:'Me’yoriy ta’minot',unitType:unit.type,unitId:unit.id,dispatcherFolderId:unit.type==='dispatcher'?unit.id:null,workZoneId:unit.type==='work_zone'?unit.id:null,unitName:meta.name},changes});closeModal();toast('Me’yoriy sonlar saqlandi va tahlil yangilandi.','success');}catch(e){modalMessage(e.message||String(e));}finally{setBusy(btn,false);}
  }
  ...
  function handleClick(e){
    ...
    if(e.target.closest('[data-se-norm-add]'))return normForm('');const norm=e.target.closest('[data-se-norm-unit]');if(norm)return normForm(norm.dataset.seNormUnit||'');
  }
  ...
  function handleChange(e){
    ...
    if(e.target.id==='hetk-se-norm-unit'){const box=byId('hetk-se-norm-rows');if(box)box.innerHTML=normQuantitiesHtml(e.target.value);}
  }
