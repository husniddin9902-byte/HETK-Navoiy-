...
// ========================================================
// 🌲 GURUHLAR DARAXTINI YIG'ISH VA BOSGANDA OCHISH MANTIQLI
// ========================================================

document.addEventListener('DOMContentLoaded', () => {
    // 1. Element paneli ochilganda papkalarni avtomat yig'ib qo'yish uslubi (CSS)
    const style = document.createElement('style');
    style.innerHTML = `
        /* Ichma-ich joylashgan guruh konteynerlarini boshida yashirib turamiz */
        #element-manage-panel .tree-branch, 
        #element-manage-panel .tree-children,
        #element-manage-panel ul ul {
            display: none;
            padding-left: 15px;
            margin-top: 5px;
        }
        /* Guruh nomi bosilganda unga pointer animatsiyasi */
        #element-manage-panel .tree-folder-title,
        #element-manage-panel li {
            cursor: pointer;
            user-select: none;
        }
    `;
    ...
    // 2. "Fider (Guruh) ni biriktirish" ro'yxatidagi har qanday guruh bosilganda ochish/yopish
    // Bu panelning ID si yoki uning ichidagi guruhlar konteynerini kuzatamiz
    const managePanel = document.getElementById('element-manage-panel');
    
    if (managePanel) {
        managePanel.addEventListener('click', function(e) {
            // Checkbox yoki input bosilsa, ochib-yopish mantiqini ishlatmaymiz (chek qo'yishga xalaqit bermasligi uchun)
            if (e.target.tagName === 'INPUT' || e.target.type === 'checkbox') {
                return;
            }

            // Bosilgan element papka yoki guruh sarlavhasi ekanligini topamiz
            const folderHeader = e.target.closest('.tree-folder-title') || e.target.closest('li');
            
            if (folderHeader) {
                // Ushbu papkaning ostidagi (ichidagi) guruhlar konteynerini qidiramiz
                // Odatda u keyingi element (nextElementSibling) yoki li ichidagi ul/div bo'ladi
                let childrenContainer = folderHeader.nextElementSibling;
                
                if (!childrenContainer || (!childrenContainer.classList.contains('tree-branch') && !childrenContainer.classList.contains('tree-children') && childrenContainer.tagName !== 'UL')) {
                    childrenContainer = folderHeader.querySelector('.tree-branch, .tree-children, ul');
                }

                if (childrenContainer) {
                    e.stopPropagation(); // Boshqa elementlarga ta'sir qilmasligi uchun

                    // Agar yopiq bo'lsa ochamiz, ochiq bo'lsa yopamiz (Toggle)
                    if (childrenContainer.style.display === 'block' || window.getComputedStyle(childrenContainer).display === 'block') {
                        childrenContainer.style.display = 'none';
                        
                        // Emoji belgisini yopiq papkaga o'zgartirish (agar kodda ishlatilgan bo'lsa)
                        folderHeader.innerHTML = folderHeader.innerHTML.replace('📂', '📁');
                    } else {
                        childrenContainer.style.display = 'block';
                        
                        // Emoji belgisini ochiq papkaga o'zgartirish
                        folderHeader.innerHTML = folderHeader.innerHTML.replace('📁', '📂');
                    }
                }
            }
